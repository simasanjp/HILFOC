...
class sizereport(Task.Task):
    ...
    def run(self):
        ...
        self.parseObjDumpOutput(bld.cmd_and_log([env.get_flat('OBJDUMP'), '-h', '-w' , self.inputs[0].abspath()], cwd=cwd, quiet=True))

        # run & parse the nm output
        self.parseNmOutput(bld.cmd_and_log([env.get_flat('NM'), '-f', 'sysv' , self.inputs[0].abspath()], cwd=cwd, quiet=True))
...
class svnexport(Task.Task):
    ...
    def run(self):
        ...
        res = self.exec_command([self.env.get_flat('SVN')] + export_options + ["export", \
