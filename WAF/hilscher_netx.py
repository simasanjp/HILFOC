...
@conf
def check_path_exists(bld, node_or_path):
    if isinstance(node_or_path, Node.Node):
        n = node_or_path.relpath()
        if os.path.exists(node_or_path.abspath()):
            node = node_or_path
        else:
            node = None
    else:
        n = node_or_path
        node = bld.path.find_node(node_or_path)

    return n, node

@conf
def check_arg_file_exists(bld, name, arg, value):
    u''' Check a path argument for existence of the spcified file '''
    value = Utils.to_list(value)

    for x in value:
        n, node = bld.check_path_exists(x)

        if not node:
            bld.fatal(u'%s: Path %r specified in arg %r for target %r does not exist' % (bld.cur_script.relpath(), n, arg, name))

        if not os.path.isfile(node.abspath()):
            bld.fatal(u'%s: Path %r specified in arg %r for target %r is not a file' % (bld.cur_script.relpath(), n, arg, name))

@conf
def check_arg_dir_exists(bld, name, arg, value):
    u''' Check a path argument for existence of the specified directory '''
    value = Utils.to_list(value)
    ...
    for x in value:
        ...
        if not node:
            bld.fatal(u'%s: Path %r specified in arg %r for target %r does not exists' % (bld.cur_script.relpath(), n, arg, name))

        if not os.path.isdir(node.abspath()):
            bld.fatal(u'%s: Path %r specified in arg %r for target %r is not a directory' % (bld.cur_script.relpath(), n, arg, name))

@conf
def check_args(bld, *args, **kw):
    u''' Perform checks on several arguments of waf build commands '''
    check_file_exists = [
      ('linkerscript',     bld.check_arg_file_exists),
      ('includes',         bld.check_arg_dir_exists),
      ('export_includes',  bld.check_arg_dir_exists),
      ('source'         ,  bld.check_arg_file_exists),
      ('source_arm'     ,  bld.check_arg_file_exists),
      ('source_thumb'   ,  bld.check_arg_file_exists)
    ]

    for arg, fun in check_file_exists:
        fun(kw['name'], arg, kw.get(arg, []))
...
def generate_mapfile(self):
    ...
    _path, _filename = os.path.split(self.target)
    _basename, _     = os.path.splitext(_filename)
    ...
    # register map file for debug distribution
    l = self.dist_debug_nodes = getattr(self, 'dist_debug_nodes', [])
    l.append(mapfile_node)

@feature('cprogram', 'cxxprogram')
@after_method('apply_link', 'propagate_uselib_vars')
def distribute_elf(self):
    u''' register the elf file for debug distribution '''

    # register elf file for debug distribution
    l = self.dist_debug_nodes = getattr(self, 'dist_debug_nodes', [])
    l.append(self.link_task.outputs[0])
    ...
    if 'patches' in kw:
        ...
        for x in Utils.to_list(kw['patches']):
            ...
            try:
                ...
                res_non_binary = (e.returncode, e.stdout, e.stderr)
                ...
                try:
                    ...
                    res_binary = (e.returncode, e.stdout, e.stderr)
                    msg = "Failed to invoke patch on %s: Non-binary failed with: %r\nBinary failed with: %r" % (x, res_non_binary, res_binary)

                    # test for know problem with wrong patch.exe on windows 10
                    if (res_binary == (1,'','')) and (res_non_binary == (1,'','')):
                        msg = msg + "\nLooks like you're using an invalid patch.exe on Windows 10. Please ensure to use a proper version of patch.exe"
...
class netx_bootimage(Task.Task):
    ...
    def get_segment_table(self, filename):
      ...
      cmd = [self.env.get_flat('OBJDUMP'), '-h', '-w', filename]
    ...
    def get_exec_address(self, filename):
      ...
      cmd = [self.env.get_flat('OBJDUMP'), '-f', filename]
      result = self.generator.bld.cmd_and_log(cmd, output=STDOUT, quiet=STDOUT)
    ...
    def run(self):
      ...
      cmd = [self.env.get_flat('OBJCOPY'), '-O', 'binary', inputfile, outputfile]
      result = self.generator.bld.cmd_and_log(cmd, output=STDOUT, quiet=STDOUT)
...
def hil_parse_version_header(self):
    u''' Process the 'version_header' argument if available '''
    bld = self.bld

    try:
        version_include = self.version_include
    except AttributeError:
        return
    ...
    for x in getattr(self_tg, "export_includes", []):
        y = self_tg.path.find_node(x)

        if y is None:
            bld.fatal("Include directory %r not found for %r" % (x, self.name))

        include_nodes.append(y)
...
@feature('*')
@after_method('hil_parse_version_header')
def hil_inherit_version(self):
    u''' Inherit version information from a use-referenced task generator '''
    bld = self.bld

    # do not overwrite already set version information
    if getattr(self,'version', None) is not None:
        return

    # Must not be a linker tgen
    if hasattr(self,'link_task'):
        return

    # Must not be an object generating tgen
    if hasattr(self,'compiled_tasks'):
        return

    l = Utils.to_list(getattr(self,'use', []))

    # Must reference exactly one other tgen
    if len(l) != 1:
        return

    tg = bld.get_tgen_by_name(l[0])

    if not getattr(tg, 'posted', False):
        tg.post()

    self.version = getattr(tg, 'version', None)
